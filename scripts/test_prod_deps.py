...
def test_production_dependencies():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        venv_result = run_command(f"python -m venv {temp_dir}/test_venv")
        ...
        if os.name == 'nt':  # Windows
            activate_script = f"{temp_dir}/test_venv/Scripts/activate"
            python_exe = f"{temp_dir}/test_venv/Scripts/python"
        else:  # Unix/Linux/MacOS
            activate_script = f"{temp_dir}/test_venv/bin/activate"
            python_exe = f"{temp_dir}/test_venv/bin/python"
        
        # Install production dependencies
        print("📦 Installing production dependencies...")
        install_cmd = f"{python_exe} -m pip install -r {project_root}/requirements.txt"
        ...
        import_result = run_command(f'{python_exe} -c "{test_script}"')
        ...
        app_result = run_command(f'{python_exe} -c "{app_test_script}"')
