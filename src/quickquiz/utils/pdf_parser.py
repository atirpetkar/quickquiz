...
class PDFParser:
    ...
    async def extract_from_url(self, url: str) -> str:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url) as response:
                    if response.status != 200:
                        raise DocumentIngestionError(f"Failed to download PDF: HTTP {response.status}")
            ...
            raise DocumentIngestionError(f"Failed to extract text from PDF URL: {str(e)}")
    ...
    async def extract_from_bytes(self, pdf_bytes: bytes) -> str:
        ...
        try:
            ...
            raise DocumentIngestionError(f"Failed to extract text from PDF bytes: {str(e)}")
    ...
    async def extract_from_file(self, file_path: str) -> str:
        ...
        try:
            ...
            raise DocumentIngestionError(f"Failed to extract text from PDF file: {str(e)}")
